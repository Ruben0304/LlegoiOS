...
public extension LlegoAPI {
  struct LoginMutation: GraphQLMutation {
    ...
    public static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"mutation Login($input: LoginInput!) { login(input: $input) { __typename accessToken tokenType user { __typename id name email phone role createdAt } } }"#
      ))
    ...
    public struct Data: LlegoAPI.SelectionSet {
      ...
      public struct Login: LlegoAPI.SelectionSet {
        @_spi(Unsafe) public let __data: DataDict
        @_spi(Unsafe) public init(_dataDict: DataDict) { __data = _dataDict }
