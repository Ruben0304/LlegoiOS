...
struct CartView: View {
    ...
    private func executePaymentProcessing(paymentMethod: PaymentMethod) {
        ...
        if backendMethod?.method.lowercased() == "cash", let method = backendMethod {
            evaluateCashAvailabilityForCheckout(
    ...
    private func applyDecision(_ decision: CashKycDecisionSnapshot) -> Bool {
        ...
        case .needsReview, .insufficientData, .error:
            setState(
                ...
                message: "Puedes reintentar la verificación o cambiar el método de pago."
            )
            return true
        case .expired:
            setState(
