...
class StoreListRepository {
    ...
    func fetchBranches(
        first: Int = 20, after: String? = nil, businessId: String? = nil,
        productCategoryId: String? = nil, radiusKm: Double? = nil,
        completion:
            @escaping @Sendable (Result<(branches: [BranchGraphQL], pageInfo: PageInfo), Error>) ->
            Void
    ) {
                        ...
                        client.fetch(query: query, cachePolicy: .returnCacheDataDontFetch) {
                            ...
                            case .success(let graphQLResult):
                                if let data = graphQLResult.data {
                                    ...
                                    print(
                                        "✅ Cargados \(mappedBranches.count) branches desde caché (offline)"
                                    )
                                    completion(
                                        .success((branches: mappedBranches, pageInfo: pageInfo)))
                                } else {
            ...
            client.fetch(query: query, cachePolicy: .returnCacheDataAndFetch) { result in
                ...
                case .failure(let error):
                                ...
                                || nsError.code == NSURLErrorNetworkConnectionLost)
                    {
                        ...
                        print(
                            "🔄 Sin conexión - Intentando cargar productos de \(branchId) desde caché..."
                        )
                        ...
                        client.fetch(query: query, cachePolicy: .returnCacheDataDontFetch) {
                            ...
                            case .success(let graphQLResult):
                                if let products = graphQLResult.data?.products {
                                    let mappedProducts = Array(products.edges.prefix(limit)).map {
                                        edge in
                                        ProductGraphQL(
                                            id: edge.node.id,
                                            branchId: edge.node.branchId,
                                    ...
                                    print(
                                        "✅ Cargados \(mappedProducts.count) productos de \(branchId) desde caché (offline)"
                                    )
                                    completion(.success(mappedProducts))
                                } else {
    ...
    func searchBranches(
        query: String, limit: Int = 10, useVectorSearch: Bool = true,
        productCategoryId: String? = nil, radiusKm: Double? = nil,
                        ...
                        if useVectorSearch {
                            ...
                            client.fetch(
                                ...
                                case .success(let fallbackGraphQLResult):
                                    ...
                                    print(
                                        "✅ Text search fallback found \(mappedBranches.count) branches"
                                    )
