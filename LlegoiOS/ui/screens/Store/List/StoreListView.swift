                            ...
                            if !searchText.isEmpty {
                                ...
                                ForEach(viewModel.stores, id: \.id) { store in
                                    StoreProductsCard(
                                        ...
                                        onBodyTap: {
                                            selectedStore = store
                                            selectedStoreGradient = nil // Or extract it if possible, but card handles it
                                            pendingDestination = .detail(store)
                                            // Usually we would navigate directly, but here we can open the sheet or navigate. 
                                            // The user said "open store profile". 
                                            // Existing logic uses selectedStore to open modal. 
                                            // But standard tap usually means "go to details".
                                            // Let's assume onBodyTap -> open Action Sheet is redundant if they tapped the body.
                                            // But wait, the StoreOptionsModal handles "View Profile". 
                                            // Let's navigate directly to details as requested, bypassing valid sheet if desired, 
                                            // OR use the established pattern. 
                                            // However, `selectedStore = store` triggers the sheet in `StoreListView`.
                                            // Let's try to navigate directly for a smoother experience if that's what "open profile" implies.
                                            // But StoreListView uses `navigationDestination`.
